...
- 用户登录后信息通过 `request.getSession().setAttribute(TOKEN_KEY, user)` 存储，Spring Session自动将会话数据持久化到Redis
- 私有接口通过 `request.getSession().getAttribute(TOKEN_KEY)` 从Session中获取用户信息进行身份验证
...
UserService -> HttpSession: setAttribute(TOKEN_KEY, user)
...
UserService -> UserService: getLoginUser(request) [从Session获取用户]
UserService -> HttpSession: getAttribute(TOKEN_KEY)
HttpSession --> UserService: 返回User对象
...
UserService -> HttpSession: getAttribute(TOKEN_KEY)
HttpSession --> UserService: 返回User对象 (命中)
...
PictureService -> UserService: getLoginUser(request) [获取当前用户]
PictureService -> HttpSession: getAttribute(TOKEN_KEY)
HttpSession --> PictureService: 返回User对象
...
PostService -> UserService: getLoginUser(request) [获取当前用户]
PostService -> HttpSession: getAttribute(TOKEN_KEY)
HttpSession --> PostService: 返回User对象
...
PictureService -> UserService: getLoginUser(request) [权限校验]
...
AuthInterceptor -> HttpSession: getAttribute(TOKEN_KEY)
HttpSession --> AuthInterceptor: 返回User对象
...
SystemController --> 后端: Response<List<String>> (["推荐", "穿搭", "美食", "旅行", "宠物", "运动"])
后端 --> 前端: 分类列表数据
前端 --> 用户: 显示分类标签
```
...
- **MySQL 8+**: 关系型数据库
- **Redis**: 缓存服务 (验证码、Session存储)
- **Spring Session**: Session管理 (基于Redis)
- **腾讯云COS**: 对象存储服务 (图片存储)
...
- **PictureController**: 图片相关接口 (2个接口)
- **SystemController**: 系统相关接口 (1个接口)

#### Service 层
- **UserService/Impl**: 用户业务逻辑 (11个方法)
- **PostService/Impl**: 帖子业务逻辑 (6个方法)
- **PictureService/Impl**: 图片业务逻辑 (3个方法)
- **CommentService/Impl**: 评论业务逻辑
- **UserPostCollectService/Impl**: 帖子收藏业务
- **UserPostLikesService/Impl**: 帖子点赞业务
- **UserFansService/Impl**: 粉丝业务逻辑
- **LikesUserByIdService/Impl**: 按用户ID查询点赞
...
Spring Boot (FishPics-backend)
  ├── 控制器层 (UserController, PostController, PictureController, SystemController)
  │     ↓ 依赖
  ├── 服务层 (UserService/Impl, PostService/Impl, PictureService/Impl等)
  │     ├── 依赖 Redis (验证码/Session存储)
  │     ├── 依赖 HttpSession (用户认证信息)
  │     ├── 依赖 Hutool (工具类)
  │     ├── 依赖 CosService (图片存储)
  │     └── 依赖
  ...
  │     ↓ 依赖
  ├── MySQL数据库 (FishPics)
  ├── Redis缓存服务器 (192.168.163.101:6379, Session存储)
  └── 腾讯云COS (对象存储服务)
```
...
| **缓存服务器** | Redis (Session存储) | 192.168.163.101:6379 |
| **对象存储** | 腾讯云COS | 云端服务 |
...
从HttpSession获取用户信息
  ↓
request.getSession().getAttribute(TOKEN_KEY)
  ↓ [Session命中]
返回User对象
  ↓ [找到用户]
...
验证管理员权限(role=admin)
  ↓ [通过 @AuthCheck]
AuthInterceptor从HttpSession获取用户信息
  ↓
校验用户角色是否为admin
  ↓ [通过]
加载用户管理页面
...
│   ├── aop (切面)
│   │   └── AuthInterceptor.java (权限拦截器, 从Session获取用户+校验角色)
│   ├── config (配置)
...
│   ├── constants (常量)
│   │   ├── RedisConstants.java (Redis键常量: LOGIN_CODE_KEY, REGISTER_CODE_KEY, TOKEN_KEY)
...
│   ├── PictureController.java (图片控制器, 2个接口)
│   └── SystemController.java (系统控制器, 1个接口)
├── dto (数据传输对象)
│   ├── base (基础请求)
...
| TOKEN_KEY | Session中存储用户信息的键 | 由Session配置决定 |
...
#### Session认证规则 (Spring Session + Redis)
- 登录成功后通过 `request.getSession().setAttribute(TOKEN_KEY, user)` 将用户信息存储到Session中
- Spring Session自动将会话数据持久化到Redis (配置: `spring.session.store-type=redis`)
- 通过 `request.getSession().getAttribute(TOKEN_KEY)` 获取用户信息进行身份验证
- Spring Session由框架管理生命周期，过期后需重新登录
- 退出登录时通过 `request.getSession().removeAttribute(TOKEN_KEY)` 清除Session中的用户信息
